    ...
    public static void main(String[] args) {
        try (LwjglPlatform platform = new LwjglPlatform()) {
            ...
            WindowProperties windowProperties = WindowProperties.builder()
                    .title("Learn OpenGL - coordinate system")
                    .size(1920, 1080)
                    .vsync(false)
                    .visible(false)
                    .resizable(true)
            ...
            try (Window window = platform.createWindow(windowProperties)) {
                ...
                try (CloseableList<Texture> textures = new CloseableList<>();
                    ...
                    Mat4 temp = new Mat4();
                    Vec3 rotationAxis = new Vec3(0.5f, 1.0f, 0.0f);
                    rotationAxis.normalizeSelf();
                    ...
                    while (!window.isCloseRequested()) {
                        ...
                        for (int i = 0; i < cubePositions.length; i++) {
                            ...
                            translationMatrix.setTranslation(cubePosition);
                            translationMatrix.translateSelf(0, 0, -3);
                            temp.setRotation(cubeRotation);
                            translationMatrix.mul(modelMatrix, temp);

                            demoMeshRenderer.draw(modelMatrix, containerTexture, faceTexture);
                        }
